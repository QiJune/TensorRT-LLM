...
| `return_perf_metrics` enabled on the LLM API (deployment arg) | pipeline never constructed — deployment-level fallback: the EngineRequest/output lack arrival_time and metrics_dict | n/a | — | FUT-10 |
| `SamplingParams(return_perf_metrics=True)` per request | co-located fallback (request-level ineligible) | headless reject | the pipeline populates no metrics_dict/arrival_time | FUT-10 |

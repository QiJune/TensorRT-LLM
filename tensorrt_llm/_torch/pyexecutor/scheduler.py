...
class SimpleUnifiedScheduler(RequestScheduler):
    ...
    def __init__(
        ...
        # Attention DP balancing/batching state (for global scheduling mode)
    ...
    def _schedule_generation_only_during_waiting(
        ...
        num_gen_tokens = sum(
            self.estimate_tokens_needed(gen_req)
    ...
    def _fused_schedule_request(
        ...
        Returns:
            UnifiedSchedulerOutput with scheduled requests
        """
        # Initialize block managers based on policy
        # These track KV cache allocation (or simulation thereof)
        ...
        # Block reuse optimization state (for capacity checking)
        skipping_is_relevant = self._is_skipping_relevant()
        ...
        if skipping_is_relevant:
            newly_contributed_context_blocks, newly_contributed_cross_context_blocks = \
                self._prefill_contributed_blocks(active_requests)
        ...
        # Micro-batch state (token budget tracking)
        batch_num_tokens = 0
        ...
        # Cache instance attributes as locals for faster access
        max_batch_size = self.max_batch_size
        max_num_tokens = self.max_num_tokens
        max_context_length = self.max_context_length
        ctx_chunk_config = self.ctx_chunk_config

        # For GUARANTEED_NO_EVICT: First pass for in-progress generation
        # (must be scheduled first to free up reserved blocks)
        if reserved_blocks is not None:
            for req in active_requests:
                if not self._can_be_scheduled_with_disagg_exception(req):
                    continue

                if len(context_requests) + len(
                        generation_requests) >= self.max_num_requests:
                    break
        ...
        # This single loop replaces the two-pass approach
        for req in active_requests:
            ...
            # Skip in-progress generation (already handled above for GUARANTEED_NO_EVICT)
            ...
            if len(context_requests) + len(generation_requests) + len(
                    fitting_disagg_gen_init) >= self.max_num_requests:
                paused_requests.append(req)
                break

            # Block reuse skip optimization
            if (skipping_is_relevant and not req.is_disagg_generation_init_state
                    and self._beneficial_to_skip(
            ...
            if req_state_value == self._encoder_init_state_value:
                ...
                can_fit_kv = self._check_kv_capacity(req,
            ...
            # --- B. Context Request Handling ---
                ...
                if not ctx_chunk_config:
                    ...
                    # Check KV cache capacity
                    can_fit_kv = self._check_kv_capacity(
        ...
        if contexts_to_be_chunked:
            ...
            if not all_context_requests_fit:
                remaining_capacity = (
                    max_num_tokens -
        ...
        self._sort_requests(context_requests, generation_requests,
                            len(contexts_to_be_chunked) > 0)

        # Return results
        num_fitting = len(context_requests) + len(generation_requests) + len(
    ...
    # ========== Helper methods for fused scheduling ==========
    # These methods are extracted from PyCapacityScheduler and PyMicroBatchScheduler
    # to support the fused single-pass scheduling approach
    ...
    def _set_ctx_requests_chunk_size(self, requests: RequestList,
        ...
        """Set chunk sizes for context requests based on chunking policy."""
    ...
    def _build_local_state(
        ...
        # Get token budget
        max_token_budget = self.max_num_tokens if self.max_num_tokens is not None else float(
            'inf')

        # Calculate current token load
        current_tokens = self.calculate_current_token_load(active_requests)

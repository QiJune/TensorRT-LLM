...
"""TensorRT-free helper for custom all-reduce workspace/IPC buffer management.

These workspace-allocation and size-computation utilities are consumed by the
PyTorch backend (``tensorrt_llm._torch``). They historically lived in the
(TensorRT-coupled) ``tensorrt_llm.plugin.plugin`` module and are relocated here
so that retained consumers can import them without pulling in the legacy
TensorRT backend.
"""
import os
...
class CustomAllReduceHelper:
    """
        ...
        This is a TensorRT-free subset of the original
        ``tensorrt_llm.plugin.plugin.CustomAllReduceHelper`` retaining only the
        workspace-allocation and size-computation utilities used by the PyTorch
        backend.
    """
    POINTERS_PER_RANK = 7

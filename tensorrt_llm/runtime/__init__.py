...
"""Surviving TensorRT-free surface of the ``tensorrt_llm.runtime`` package.

The legacy TensorRT Python runtime (``GenerationSession``, ``Session``,
``ModelRunner``/``ModelRunnerCpp``, the multimodal/enc-dec runners and the v1
``KVCacheManager``) was removed with the TensorRT backend. The retained surface
is the backend-agnostic ``ModelConfig`` and the modern ``kv_cache_manager_v2``.
"""

...
"""Runtime ``ModelConfig`` dataclass.

This dataclass is shared with the PyTorch backend (it is imported as
``ModelConfigPython`` by ``_torch/pyexecutor/resource_manager.py``). It lives in
its own TensorRT-free module so that those consumers do not have to import the
legacy TensorRT runtime in ``generation.py``.
"""

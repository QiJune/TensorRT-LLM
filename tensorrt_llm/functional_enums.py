...
"""Backend-agnostic functional type definitions shared across backends.

These enums, parameter classes, and rotary-embedding helpers are consumed by the
PyTorch backend (``tensorrt_llm._torch``) and VisualGen but historically lived in
the (TensorRT-coupled) ``tensorrt_llm.functional`` module. They are relocated here
so that retained consumers can import them without pulling in the legacy TensorRT
backend (which depends on the ``tensorrt`` Python package and ``onnx_graphsurgeon``).

All definitions in this module are TensorRT-free (numpy/torch only). Annotations
are kept lazy via ``from __future__ import annotations`` so that references to the
legacy TensorRT ``Tensor`` type in copied signatures do not require importing it.
"""

...
"""TensorRT-free runtime initialization for the package.

This replaces the legacy ``tensorrt_llm._common._init()`` once the TensorRT
backend is removed. It performs only the initialization the retained PyTorch
backend needs: loading the ``libth_common.so`` custom-op library, registering
the fake (meta) kernels, and initializing the MPI communicator. It does NOT
load the TensorRT plugin library (``libnvinfer_plugin_tensorrt_llm.so``).
"""
...
def _init_runtime(log_level: object = None) -> None:
    global _inited
    if _inited:
        return
    _inited = True

...
After the TensorRT backend removal, ``tensorrt_llm.bindings.DataType`` is backed
by the standalone ``tensorrt_llm::DataType`` enum (``common/tllmDataType.h``)
instead of ``nvinfer1::DataType``. Its enumerator *integer values* must continue
to match the legacy ``nvinfer1::DataType`` values so that previously-serialized
executor configs and KV-cache metadata remain byte-compatible, and so the
public Python member set is unchanged.
"""

...
struct QKVPreprocessingParams
{
    ...
    std::string toString() const
    {
        ...
        if (cache_seq_lens && batch_size > 0)
        {
            ss << "cache_seq_lens: "
               << *(runtime::ITensor::wrap(
                      (void*) cache_seq_lens, tensorrt_llm::DataType::kINT32, runtime::ITensor::makeShape({batch_size})));
        }
        if (encoder_seq_lens && batch_size > 0)
        {
            ss << "encoder_seq_lens: "
               << *(runtime::ITensor::wrap(
                      (void*) encoder_seq_lens, tensorrt_llm::DataType::kINT32, runtime::ITensor::makeShape({batch_size})));
        }
        ...
        if (cu_kv_seq_lens && batch_size > 0)
        {
            ss << "cu_kv_seq_lens: "
               << *(runtime::ITensor::wrap(
                      (void*) cu_kv_seq_lens, tensorrt_llm::DataType::kINT32, runtime::ITensor::makeShape({batch_size})));
        }

...
inline void runMoeLoraDeviceModule(::tensorrt_llm::kernels::cutlass_kernels::MoeLoraDevicePathModule const& mod,
    ...
    ::tensorrt_llm::kernels::cutlass_kernels::MoeLoraDeviceRunFn run, tensorrt_llm::DataType data_type, cudaStream_t stream)
{
    ...
    LoraParams& lora_params, float const* input_fp8_dequant, cudaStream_t stream) -> ScaleBiasType const*
{
    ...
    {
        auto const& dp = lora_params.device_path;
        tensorrt_llm::DataType const data_type = moeLoraNvInferType<ScaleBiasType>();
